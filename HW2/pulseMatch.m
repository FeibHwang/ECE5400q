...
if 1
    % this bit is the solution to the problem
    
    bb=find(diff(cnet(:,3))==0)+1; % returns the index of all repeated pulses, the +1 to account for the fact that diff(x) has one less row than x
    bb2=[bb;bb-1]; % want to get rid of both repeated pulses
    bb2=[bb2;bb-2+3*cnet(bb,3)]; % want to mark the previous upstream pulse if the unmatched pair is downstream, or mark the following downstream pulse if the unmatched pair is upstream. Using bb2 so that the previous change does not disrupt things at this step. Also note, if there are three repeated pulses this code will still work... think about why and if you can't figure it out, ask me about it
    bb=bb2;
    if cnet(1,3)==0
        bb=[1;bb]; % we do not want to keep the first pulses if it is downstream... make sure you know why
    end
    if cnet(end,3)==1
        bb=[bb;size(cnet,1)]; % likewise, we do not want to keep the last transition if it is upstream
    end
    cnet(bb,:)=[]; % delete the marked rows
    
    cuu=cnet(cnet(:,3)==1,1:2);
    cdd=cnet(cnet(:,3)==0,1:2);
    
    pu=[1+0*cuu(:,1),cuu(:,1);0*cuu(:,2),cuu(:,2)];
    pu=sortrows(pu,2);
    
    pd=[1+0*cdd(:,1),cdd(:,1);0*cdd(:,2),cdd(:,2)];
    pd=sortrows(pd,2);
end
